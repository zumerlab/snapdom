 ...
 */
function shrinkAutoSizeBoxes(sourceRoot, cloneRoot, styleCache = new Map()) {
  /**
   * @param {Element} src
   * @param {Element} cln
   */
...
function willBeExcluded(el, options) {
  ...
  // If you also remove via filter:
  // if (typeof options?.filter === 'function' && options.filterMode === 'remove') { try { if (!options.filter(el)) return true } catch {} }
  return false
}
