...
describe('snapdom advanced tests', () => {
  ...
  it('should replace iframes with fallback visuals', async () => {
    ...
    // Verificamos que NO haya un iframe adentro
    const iframesInSvg = doc.querySelectorAll('iframe');
    expect(iframesInSvg.length).toBe(0);
  
    // Y opcionalmente chequeamos que haya un div de fallback
    const fallbackDivs = Array.from(doc.querySelectorAll('div'))
      .filter(div => div.style && div.style.background?.includes('repeating-linear-gradient'));
