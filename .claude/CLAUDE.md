...
WICG html-in-canvas (`ctx.drawElementImage`; the M138-era dev trial called it `drawElement` and the module detects both, OT name first), opt-in via `engine: 'html-in-canvas'`. Fully quarantined: core's only knowledge is a 3-line lazy import in `snapdom.js`, and on ANY doubt `tryEngineResult` returns null and the normal pipeline runs. Status 2026-09-03: the spec dropped unconditional tainting for "read-back-allowed rendering" — sensitive/cross-origin content is excluded from painting and readback is allowed — shipping in the Chrome 148–154 origin trial behind `chrome://flags/#canvas-draw-element`; no Intent to Ship yet. **VERIFIED ACTIVE the same day in real Chrome M148+ with the flag**: `drawElementImage` detected, clone painted, taint probe passed, capture returned PNG — the happy path runs on real hardware. The engine follows the OT contract (`layoutsubtree` on the canvas, `drawable` on the mounted wrapper, `paint` event after `requestPaint()` as the sync point, bounded) and draws the FINISHED CLONE — whose cross-origin content the pipeline already inlined, making it same-origin by construction, so the native-content exclusions don't bite and the taint probe passes. Unflagged browsers still fall through to the svg engine at `detectDrawApi()`. Timing on real flagged Chrome M148+ with GPU (2026-09-03): a small card is svg 1.7 ms vs engine 33.2 ms — the engine's floor is its two-frame mount sync (~2×16 ms), so tiny captures stay svg territory; the engine's case is big/complex captures, where it skips the SVG decode entirely (bitmap handoff measured 4.2x the old PNG round trip at 8.4 Mpx). A possible future trim: with `requestPaint`/`paint` available the double-rAF may be partly redundant — measure before touching. Keep the quarantine contract intact — correctness must never depend on this module.
