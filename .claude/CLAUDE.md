...
- Memoization engages on an element's FIRST capture, capped at 64 live memos with LRU teardown (`disposeState`) and a `:hover` chain compared on every serve: the notes above `MAX_LIVE_MEMOS` and `hoverChain`, `src/core/burst.js`. `snapdom.preCapture()` is the only prefetch: link prefetch translated (intent on a control captures the element that control asked for last time; the first intent warms the viewport), `src/api/preCapture.js`. No `preCache`, no attribute, no re-seed, no document-level warm, by decision (2026-09-04); the corpus numbers behind it (79 demos: first capture 20.4 ms median, steady 4.8, hit 0, export 4.6) are in the header of that file.
